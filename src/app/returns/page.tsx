    ...
    <div className="min-h-screen py-16 lg:py-24 px-4">
      <div className="container-normal mx-auto max-w-4xl space-y-12">
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <p className="text-muted-foreground text-sm font-light uppercase tracking-widest text-center">We value your satisfaction and the integrity of our art.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {policies.map((policy, idx) => {
            ...
            return (
              <div key={idx} className="bg-secondary/60 backdrop-blur-md p-8 rounded-[2.5rem] border border-primary/5 shadow-sm space-y-4 text-center">
                <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center mx-auto text-primary">
                  <Icon className="h-6 w-6" />
                </div>
                <h3 className="text-lg font-black uppercase tracking-tight text-foreground">{policy.title}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed font-light">{policy.content}</p>
              </div>
            );
          })}
        </div>

        <div className="bg-primary/10 p-12 rounded-[4rem] text-center space-y-6">
          <h3 className="text-2xl font-black uppercase tracking-widest text-primary-foreground">Need Assistance?</h3>
          <p className="text-primary-foreground/60 max-w-lg mx-auto font-light">Our artist is here to help you with any concerns regarding your order or the crafting process.</p>
          <Link href="/contact">
