...
export function DesktopCategoryGrid() {
  const artisticCategories = categoriesData.categories;
        ...
        <div className="grid grid-cols-7 gap-4">
          {artisticCategories.map((cat, index) => (
            ...
            >
              ...
              <span className="block text-xs font-bold text-muted-foreground group-hover:text-primary transition-colors leading-tight px-1 uppercase tracking-widest">
                {cat.name}
              </span>
            </Link>
          ))}
